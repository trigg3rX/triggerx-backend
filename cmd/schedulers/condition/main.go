...
func main() {
	...
	if redisx.IsAvailable() {
		...
		if err != nil {
			...
			startupEvent := map[string]interface{}{
				...
				"message":      "Condition scheduler service started with enhanced Redis",
			}
			...
			if err := redisx.AddJobToStream(redisx.JobsReadyConditionStream, startupEvent); err != nil {
				...
				logger.Info("Startup event added to Redis condition stream successfully")
			}
		}
	} else {
		logger.Warn("Redis not configured - condition streaming disabled")
	...
	if err := cache.InitWithLogger(logger); err != nil {
		...
		if err != nil {
			...
			testPerformerID := "test_condition_startup_performer"
	...
	performGracefulShutdown(cancel, srv, conditionScheduler, redisClient, logger)
}
...
func performGracefulShutdown(cancel context.CancelFunc, srv *api.Server, conditionScheduler *scheduler.ConditionBasedScheduler, redisClient *redisx.Client, logger logging.Logger) {
