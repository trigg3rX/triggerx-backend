...
func NewCollector(metrics observability.Metrics) *Collector {
	...
	if metrics != nil {
		...
		if promMetrics, ok := metrics.(prometheusMetrics); ok {
			...
			// Note: In this architecture, metrics are registered via InitializeMetrics
			// But if we have specific Prometheus collectors that need manual registration, we'd do it here.
			// Since InitializeMetrics uses the observability.Metrics interface which wraps the registry,
			// we just need to access the handler.
