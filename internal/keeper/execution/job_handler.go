...
// 	logger.Info("[JobHandler] 🔧 Received job %d for execution", job.JobID)

// 	// Validate job
// 	if err := h.validateJob(job); err != nil {
// 		logger.Error("[JobHandler] ❌ Job validation failed: %v", err)
// 		return err
// 	}
...
// 	if err != nil {
// 		logger.Error("[JobHandler] Error fetching checker template: %v", err)
// 		return err
// 	}
...
// 	if codeURL == "" {
// 		logger.Info("[JobHandler] No CodeURL provided, skipping IPFS fetch")
// 		return "", nil
// 	}
...
// 	logger.Info("[JobHandler] Executing contract call for job %d with no arguments", job.JobID)
...
// 	if err != nil {
// 		logger.Error("[JobHandler] Error packing input for method %s: %v", method.Name, err)
// 		return err
// 	}
// 	logger.Info("[JobHandler] Packed input for method %s: %x", method.Name, input)
...
// 	}, nil)
// 	if err != nil {
// 		logger.Error("[JobHandler] Error calling contract: %v", err)
// 		return err
// 	}
...
// 	if err != nil {
// 		logger.Error("[JobHandler] Error decoding contract output: %v", err)
// 		return err
// 	}

// 	logger.Info("[JobHandler] ✅ Job %d executed successfully. Result: %+v", job.JobID, map[string]interface{}{
...
// 	logger.Info("[JobHandler] Executing contract call for job %d with static arguments", job.JobID)
...
// 	if err != nil {
// 		logger.Error("[JobHandler] Error packing arguments: %v", err)
// 		return err
// 	}
...
// 	}, nil)
// 	if err != nil {
// 		logger.Error("[JobHandler] Contract call error: %v", err)
// 		return err
// 	}
...
// 	logger.Info("[JobHandler] Executing contract call for job %d with dynamic arguments", job.JobID)
...
// func (h *JobHandler) loadCheckerFunction(job *types.Job) (func(*types.Job) (bool, map[string]interface{}), error) {
//     logger.Info("Loading checker function for job %s", job.JobID)

//     // If no code URL is provided, return a default checker
//     if job.ScriptIPFSUrl == "" {
//         logger.Info("No CodeURL provided, using default checker")
...
//     switch job.ArgType {
//     case 0, 1:
//         return func(j *types.Job) (bool, map[string]interface{}) {
...
//             output, err := cmd.CombinedOutput()
//             if err != nil {
//                 logger.Error("Error running checker template: %v", err)
//                 return false, make(map[string]interface{})
//             }
...
//             if err != nil {
//                 logger.Error("Error parsing checker output: %v", err)
//                 return false, make(map[string]interface{})
//             }

//             // Check if payload contains a price
//             if _, ok := payload["price"]; !ok {
//                 logger.Error("No price found in payload")
//                 return false, make(map[string]interface{})
//             }
...
// func (h *JobHandler) defaultChecker(argType int) func(*types.Job) (bool, map[string]interface{}) {
//     return func(j *types.Job) (bool, map[string]interface{}) {
//         switch argType {
//         case 0, 1:
//             if h.intervalChecker.ValidateJobInterval(j) {
...
// 	logger.Info("Sending value to Quorum Head: %v", originalValue)
...
// 	if err != nil {
// 		logger.Error("Error in Quorum Head processing: %v", err)
// 		return nil, err
// 	}
...
// 	logger.Info("Receiving value from workers: %v", originalValue)

// 	// Placeholder for worker value processing
// 	consensusValue, err := h.runConsensus(originalValue)
// 	if err != nil {
// 		logger.Error("Error in worker value processing: %v", err)
// 		return nil, err
// 	}
...
// 	logger.Info("Running consensus on value: %v", originalValue)
...
// 	if !exists {
// 		logger.Info("No URL provided for dynamic argument, returning default value 0")
// 		return "0", nil
// 	}
...
// 	if !ok {
// 		logger.Info("Invalid URL format, returning default value 0")
// 		return "0", nil
// 	}

// 	logger.Info("Fetching dynamic argument from URL: %s", url)

// 	// Attempt to fetch value from API
// 	resp, err := http.Get(url)
// 	if err != nil {
// 		logger.Error("Error fetching dynamic argument: %v, returning default value 0", err)
// 		return "0", nil
// 	}
...
// 	if err != nil {
// 		logger.Error("Error reading response body: %v, returning default value 0", err)
// 		return "0", nil
// 	}

// 	logger.Info("Received API response body: %s", string(body))

// 	// Flexible response parsing
// 	var apiResponse map[string]interface{}
// 	err = json.Unmarshal(body, &apiResponse)
// 	if err != nil {
// 		logger.Error("Error parsing API response: %v, returning default value 0", err)
// 		return "0", nil
// 	}
...
// 	default:
// 		logger.Info("Unexpected value type: %T, returning default value 0", value)
// 		return "0", nil
// 	}

// 	// If no value is found or conversion fails, return "0"
// 	if stringValue == "" {
// 		logger.Info("No value found in API response, returning default value 0")
// 		return "0", nil
// 	}
...
// 	if err != nil {
// 		logger.Error("Error in Quorum Head processing: %v, using original value", err)
...
// 		logger.Info("Updated data to: %v", processedValue)
// 	}

// 	logger.Info("Fetched and processed dynamic argument value: %v", processedValue)
...
// 		logger.Info("[JobHandler] Method %s has no outputs to decode", method.Name)
// 		return nil, nil
// 	}
...
// 		if err != nil {
// 			logger.Error("[JobHandler] Error unpacking single output: %v", err)
// 			return nil, err
// 		}

// 		logger.Info("[JobHandler] Decoded single output: %v", result.Interface())   
...
// 	if err != nil {
// 		logger.Error("[JobHandler] Error unpacking multiple outputs: %v", err)
// 		return nil, err
// 	}

// 	logger.Info("[JobHandler] Decoded multiple outputs: %+v", results)
// 	return results, nil
// }
...
// 	if err != nil {
// 		logger.Error("[JobHandler] Error parsing ABI: %v", err)
// 		return nil, nil, err
// 	}

// 	logger.Info("[JobHandler] Fetched ABI for contract %s: %s", contractAddress, string(abiData))

// 	method, ok := parsed.Methods[methodName]
// 	if !ok {
// 		logger.Error("[JobHandler] Method %s not found in contract ABI", methodName)
// 		return nil, nil, fmt.Errorf("method %s not found in contract ABI", methodName)
// 	}

// 	logger.Info("[JobHandler] Found method: %+v", method)
