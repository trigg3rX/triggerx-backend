...
func NewTaskStreamManager(ctx context.Context, redisClient *redis.Client, taskRepo repository.TaskRepository, logger observability.Logger, tracer observability.Tracer, keeperClient KeeperClientInterface) (*TaskStreamManager, error) {
...
func (tsm *TaskStreamManager) RegisterConsumerGroup(ctx context.Context, stream string, group string) error {
	...
	if err := tsm.redisClient.GetUnderlyingClient().CreateConsumerGroup(ctx, stream, group); err != nil {
...
func (tsm *TaskStreamManager) GetTaskRepository() repository.TaskRepository {
	return tsm.taskRepo
}
...
func (tsm *TaskStreamManager) GetPendingEntriesInfo(ctx context.Context) map[string]interface{} {
	...
	for _, group := range consumerGroups {
		pending, err := tsm.redisClient.GetPendingTasks(ctx, types.StreamTaskDispatched, group)
		if err != nil {
...
func (tsm *TaskStreamManager) CleanupPendingEntries(ctx context.Context) error {
	...
	for _, group := range consumerGroups {
		...
		pendingExt, err := tsm.redisClient.GetPendingTasksExt(ctx, types.StreamTaskDispatched, group, 100)
		if err != nil {
		...
		for _, entry := range pendingExt {
			...
			if entry.Idle > time.Hour {
				err := tsm.redisClient.AcknowledgeTask(ctx, types.StreamTaskDispatched, group, entry.ID)
				if err != nil {
...
func (tsm *TaskStreamManager) TrimStreams(ctx context.Context) error {
	...
	for stream, maxLen := range streamConfigs {
		trimmed, err := tsm.redisClient.TrimStream(ctx, stream, maxLen, true) // Use approximate trimming for better performance
		if err != nil {
...
func (tsm *TaskStreamManager) CleanupExpiredStreamEntries(ctx context.Context) error {
	...
	// Get expired entries for all streams using the redis client
	expiredEntries, err := tsm.redisClient.GetExpiredEntriesForAllStreams(ctx)
	if err != nil {
	...
	for stream, messageIDs := range expiredEntries {
		...
		// Remove from expiration tracking using the redis client
		err = tsm.redisClient.RemoveMultipleStreamExpirations(ctx, stream, messageIDs)
		if err != nil {
