...
import (
	...
	"github.com/trigg3rX/triggerx-backend/internal/eventmonitor/core/attestation"
	"github.com/trigg3rX/triggerx-backend/internal/eventmonitor/core/registry"
	"github.com/trigg3rX/triggerx-backend/internal/eventmonitor/core/worker"
	"github.com/trigg3rX/triggerx-backend/internal/eventmonitor/rpc/clients/conditionscheduler"
...
func NewService(ctx context.Context, logger observability.Logger, tracer observability.Tracer) (*Service, error) {
	...
	csClient := conditionscheduler.NewClient(logger, tracer)
	...
	return &Service{
		...
		conditionSchedulerClient: csClient,
...
func (s *Service) startWorker(key string) error {
	...
	// Create worker
	w := worker.NewWorker(entry, nodeClient, s.conditionSchedulerClient, s.logger, s.tracer)
	...
	for key, entry := range entries {
		if _, exists := s.workers[key]; !exists {
			...
			if _, exists := s.nodeClients[entry.ChainID]; exists {
				// Start worker
				w := worker.NewWorker(entry, s.nodeClients[entry.ChainID], s.conditionSchedulerClient, s.logger, s.tracer)
				s.workers[key] = w
				s.wg.Add(1)
