...
check_manager_api() {
  ...
  docker exec -i triggerx-manager-$((port - 6060)) netstat -tulpn | grep LISTEN
}
...
test_manager_health() {
  ...
  check_manager_api 6061
  check_manager_api 6062
  
  # Check Manager 1
  log_info "Checking Manager 1 health..."
  local manager1_health=$(curl -s -v http://localhost:6061/health 2>&1)
  ...
  while ! echo "$manager1_health" | grep -q "ok" && [ $retry_count -lt $max_retries ]; do
    ...
    sleep 10
    manager1_health=$(curl -s -v http://localhost:6061/health 2>&1)
  ...
  local manager2_health=$(curl -s -v http://localhost:6062/health 2>&1)
  ...
  while ! echo "$manager2_health" | grep -q "ok" && [ $retry_count -lt $max_retries ]; do
    ...
    sleep 10
    manager2_health=$(curl -s -v http://localhost:6062/health 2>&1)
...
test_manager_leadership() {
  ...
  leader1=$(curl -s http://localhost:6061/status | grep -o '"isLeader":[^,]*' || echo "Failed to get status")
  leader2=$(curl -s http://localhost:6062/status | grep -o '"isLeader":[^,]*' || echo "Failed to get status")
