...
func TestFailingMockFS(t *testing.T) {
	failingFS := &FailingMockFS{
		MockFileSystem: *NewMockFileSystem(),
	}

	// Test that it inherits from MockFileSystem
	var _ FileSystemAPI = failingFS

	// Test that MkdirAll fails
	err := failingFS.MkdirAll("/test/path", 0755)
	if err == nil {
		t.Error("FailingMockFS.MkdirAll should return an error")
	}
	...
	// Test that other methods still work (inherited from MockFileSystem)
	err = failingFS.WriteFile("test.txt", []byte("test"), 0644)
	if err != nil {
		t.Errorf("WriteFile should work on FailingMockFS: %v", err)
	}

	content, err := failingFS.ReadFile("test.txt")
	if err != nil {
		t.Errorf("ReadFile should work on FailingMockFS: %v", err)
	}
