...
func TestExecutionFeeConfig_Validate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "NegativeDynamicComplexityFactor_ShouldFail",
			config: ExecutionFeeConfig{
				PricePerTG:      0.0001,
				FixedCost:       1.0,
				TransactionCost: 1.0,
			...
			wantErr: true,
			errMsg:  "dynamic_complexity_factor cannot be negative",
		},
		{
			name: "NegativeStaticComplexityFactor_ShouldFail",
			config: ExecutionFeeConfig{
				PricePerTG:      0.0001,
				FixedCost:       1.0,
				TransactionCost: 1.0,
			...
			wantErr: true,
			errMsg:  "static_complexity_factor cannot be negative",
		},
	}
