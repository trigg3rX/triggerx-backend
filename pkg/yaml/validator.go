...
// validateDuration validates duration strings or Duration types
func (v *Validator) validateDuration(field reflect.Value, fieldType reflect.StructField) error {
	// Check if it's a Duration type from pkg/yaml
	if field.Type() == reflect.TypeOf(Duration{}) {
		// Duration type already validates during unmarshaling, so just check it's not zero
		durationField := field.Interface().(Duration)
		if durationField.Duration == 0 {
			return fmt.Errorf("duration field %s cannot be zero", fieldType.Name)
		}
		return nil
	}

	// Fallback to string validation for backward compatibility
	if field.Kind() != reflect.String {
		return fmt.Errorf("duration field %s must be a string or yaml.Duration", fieldType.Name)
	}
