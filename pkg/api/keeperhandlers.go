...
func (h *Handler) GetKeeperPeerInfo(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&peerInfo); err != nil {
		h.logger.Error("[GetKeeperPeerInfo] Error decoding request body: %v", err)
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}

	h.logger.Info("[GetKeeperPeerInfo] Retrieving peer info for keeper ID: %s", peerInfo.ID)
	if err := h.db.Session().Query(`
		...
		h.logger.Error("[GetKeeperPeerInfo] Error retrieving peer info for keeper ID %s: %v", peerInfo.ID, err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}

	h.logger.Info("[GetKeeperPeerInfo] Successfully retrieved peer info for keeper ID: %s", peerInfo.ID)
...
func (h *Handler) CheckKeeperRegistration(w http.ResponseWriter, r *http.Request) {
	...
	h.logger.Info("[CheckKeeperRegistration] Retrieving keeper by address: %s", keeperAddress)
	...
	if err := h.db.Session().Query(`
		...
		h.logger.Error("[CheckKeeperRegistration] Error retrieving keeper by address %s: %v", keeperAddress, err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}

	if keeper.KeeperID == 0 {
		h.logger.Error("[CheckKeeperRegistration] No keeper found for address: %s", keeperAddress)
	...
	h.logger.Info("[CheckKeeperRegistration] Successfully retrieved keeper by address: %s", keeperAddress)
	w.WriteHeader(http.StatusOK)
...
func (h *Handler) CreateKeeperData(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&keeperData); err != nil {
		h.logger.Error("[CreateKeeperData] Error decoding request body: %v", err)
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	...
	if !validationStatus {
		h.logger.Error("[CreateKeeperData] Registration validation failed for keeper ID: %d", keeperData.KeeperID)
		http.Error(w, "Registration validation failed", http.StatusBadRequest)
		return
	}

	h.logger.Info("[CreateKeeperData] Creating keeper with ID: %d", keeperData.KeeperID)
	if err := h.db.Session().Query(`
		...
		h.logger.Error("[CreateKeeperData] Error creating keeper with ID %d: %v", keeperData.KeeperID, err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}

	h.logger.Info("[CreateKeeperData] Successfully created keeper with ID: %d", keeperData.KeeperID)
...
func (h *Handler) GetKeeperData(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	keeperID := vars["id"]
	h.logger.Info("[GetKeeperData] Retrieving keeper with ID: %s", keeperID)
	...
	if err := h.db.Session().Query(`
		...
		h.logger.Error("[GetKeeperData] Error retrieving keeper with ID %s: %v", keeperID, err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}

	h.logger.Info("[GetKeeperData] Successfully retrieved keeper with ID: %s", keeperID)
	json.NewEncoder(w).Encode(keeperData)
}

func (h *Handler) GetAllKeepers(w http.ResponseWriter, r *http.Request) {
	h.logger.Info("[GetAllKeepers] Retrieving all keepers")
	...
	if err := iter.Close(); err != nil {
		h.logger.Error("[GetAllKeepers] Error retrieving keepers: %v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}

	h.logger.Info("[GetAllKeepers] Successfully retrieved %d keepers", len(keepers))
...
func (h *Handler) UpdateKeeperData(w http.ResponseWriter, r *http.Request) {
	...
	var keeperData types.KeeperData
	if err := json.NewDecoder(r.Body).Decode(&keeperData); err != nil {
		h.logger.Error("[UpdateKeeperData] Error decoding request body for keeper ID %s: %v", keeperID, err)
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}

	h.logger.Info("[UpdateKeeperData] Updating keeper with ID: %s", keeperID)
	if err := h.db.Session().Query(`
		...
		h.logger.Error("[UpdateKeeperData] Error updating keeper with ID %s: %v", keeperID, err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}

	h.logger.Info("[UpdateKeeperData] Successfully updated keeper with ID: %s", keeperID)
	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(keeperData)
}
...
func (h *Handler) CreateTaskHistory(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&taskHistory); err != nil {
		h.logger.Error("[CreateTaskHistory] Error decoding request body: %v", err)
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}

	h.logger.Info("[CreateTaskHistory] Creating task history for task ID: %d", taskHistory.TaskID)
	if err := h.db.Session().Query(`
		...
		h.logger.Error("[CreateTaskHistory] Error creating task history for task ID %d: %v", taskHistory.TaskID, err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}

	h.logger.Info("[CreateTaskHistory] Successfully created task history for task ID: %d", taskHistory.TaskID)
	w.WriteHeader(http.StatusCreated)
...
func (h *Handler) GetTaskHistory(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	taskID := vars["id"]
	h.logger.Info("[GetTaskHistory] Retrieving task history for task ID: %s", taskID)
	...
	if err := h.db.Session().Query(`
		...
		h.logger.Error("[GetTaskHistory] Error retrieving task history for task ID %s: %v", taskID, err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}

	h.logger.Info("[GetTaskHistory] Successfully retrieved task history for task ID: %s", taskID)
	json.NewEncoder(w).Encode(taskHistory)
}
...
func checkRegistration(h *Handler, registeredTx string, withdrawalAddress string) bool {
	...
	if err := godotenv.Load(); err != nil {
		h.logger.Error("[checkRegistration] Error loading .env file: %v", err)
	}

	// Get ETH RPC URL from environment
	ethRPCURL := os.Getenv("ETH_RPC_URL")
	if ethRPCURL == "" {
		h.logger.Error("[checkRegistration] ETH_RPC_URL environment variable not set")
		return false
	}
	...
	if err != nil {
		h.logger.Error("[checkRegistration] Failed to connect to Ethereum client: %v", err)
		return false
	}
	defer client.Close()
	...
	if err != nil {
		h.logger.Error("[checkRegistration] Failed to get transaction: %v", err)
		return false
	}

	if isPending {
		h.logger.Error("[checkRegistration] Transaction is still pending")
		return false
	}
	...
	if err != nil {
		h.logger.Error("[checkRegistration] Failed to get transaction sender: %v", err)
		return false
	}

	// Compare addresses (case-insensitive)
	if !strings.EqualFold(from.Hex(), withdrawalAddress) {
		h.logger.Error("[checkRegistration] Address mismatch - From: %s, Expected: %s", from.Hex(), withdrawalAddress)
		return false
	}
